        ...
        foreach (var c in package.Classes)
        {
            ...
            foreach (var m in c.Methods)
            {
                ...
                ListMethods.Add(new MethodData
                {
                    ...
                    BranchCoveragePercent = m.BranchRate
                });
            }

            ListMethods = [.. ListMethods.OrderBy(m => m.Name)];
            ...
            ListClasses.Add(new ClassData
            {
                ...
                BranchCoveragePercent = c.BranchRate,
                TotalLines = c.Methods.Select(m => m.Lines.Count).Sum(),
                CoveredLines = c.Methods.SelectMany(m => m.Lines).Count(l => l.Hits > 0),
                DateRetrieved = repoData.DateRetrieved,
                ListMethods = ListMethods
            });
        }

        ListClasses = [.. ListClasses.OrderBy(c => c.Name)];
